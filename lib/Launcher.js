...
const { dockerUpdateChrome } = require('../src/utils/dockerChrome');
...
module.exports = class Launcher {
    constructor({
        ...
        if (this.dockerChromeVersion) {
            dockerUpdateChrome(this.dockerChromeVersion);
        }
    ...
    async runTests() {
        return new Promise(resolve => {
            this.mocha.run(async err => {
                this.testController.closeBrowser();
                if (this.onFinish) {
