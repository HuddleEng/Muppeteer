...
const waits = require('../src/api/waits');
const keyboard = require('../src/api/keyboard');
const retrieval = require('../src/api/retrieval');
const visual = require('../src/api/visual');
const serialization = require('../src/api/serialization');
const {browserInstance, debugMode} = require('./test-controller');
    ...
    [createPageAPI]() {
        ...
        let api = Object.assign({}, {
                async turnOffAnimations () {
                    return await self._puppeteerPage.evaluate(() => {
                ...
                async evaluate(fn, ...args) {
                    ...
                    return await self._puppeteerPage.evaluate(fnStr);
                },
                async focus(selector) {
                    return await self._puppeteerPage.focus(selector);
                },
                async isElementFocused (selector) {
                    return await self._puppeteerPage.evaluate(selector => {
                        const element = document.querySelector(selector);
                        return element === document.activeElement;
                    }, selector);
                },
                async type(selector, text) {
                    return await self._puppeteerPage.type(selector, text);
                },
                async click(selector) {
                    return await self._puppeteerPage.click(selector);
                },
                async setViewport(viewport) {
                    return await self._puppeteerPage.setViewport(viewport);
                },
            },
        ...
        assert.visual = async function(selector) {
            ...
            return assert.equal(r.result, 'pass', `Visual failure: { selector: ${selector}, misMatchPercentage: ${r.misMatchPercentage} }`);
        };
