...
const dockerBuild = async () => {
    try {
        ...
        await runCommand('docker-compose', ['-f', `"${composePath}"`, 'build']);
...
module.exports = {
    browserInstance: {
        ...
        async launch({headless = true, webSocketUri = '', disableSandbox = false, executablePath = null, useDocker = true} = {}) {
            if (webSocketUri) {
                isUsingDocker = useDocker;
                browser = await puppeteer.connect({browserWSEndpoint: webSocketUri});
                return browser;
            }
            ...
            if (useDocker) {
                ...
                console.log(`${CONSOLE_PREFIX} Connected to WebSocket URL: ${webSocketUri}`.green);

                if (process.send) {
                    process.send({tag: 'STDOUT_HOOK_WS', value: webSocketUri});
                }
