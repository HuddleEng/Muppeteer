...
const { globMatch, legacyMatch } = require('../src/test-filtering');
...
module.exports = async function ConfigureLauncher({
    ...
    if (dockerChromeVersion) {
        ...
        shell.exec(
            `cd "${muppeteerPath}" && ${prefix}update-docker-chrome.sh ${dockerChromeVersion}`
        );
    }
    ...
    const files = testPathPattern
        ? await globMatch(testPathPattern)
