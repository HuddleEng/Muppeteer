...
const Mocha = require('../node_modules/jest-runner-mocha/build/mocha');
...
module.exports = function ConfigureLauncher({
    ...
    Mocha.interfaces.muppeteer = muppeteerInterface(
        ...
        componentTestVisualPathFactory ||
            (component => path.join(testDir, `/screenshots/${component.name}`)),
        visualThreshold,
        shouldRebaseVisuals
    );
    ...
    const init = existingWebSocketUri => {
        console.log('launching at ', existingWebSocketUri);
    ...
    const runTests = () => {
        ...
        return new Promise(async resolve => {
            try {
                //     const { stdout, stderr } = await exec('jest example/*');
